  ...
  return (
    <Layout location={location} title={siteTitle}>
      ...
      <article className="post">
        ...
        <time dateTime={post.frontmatter.date} className="post-date">
          ...
          {post.frontmatter.categories &&
            post.frontmatter.categories.map((category, index, categories) => (
              <React.Fragment key={index}>
                {index === 0 && <>&nbsp;in </>}
                <Link to={`/categories/#${slugify(category, { lower: true })}`}>
                  {category}
                </Link>
          ...
          {post.frontmatter.last_modified_at && (
            <>
              <br />
              <i>Updated: {post.frontmatter.last_modified_at}</i>
            </>
          )}
        </time>
      ...
      <div className="PageNavigation">
        {previous && (
          <Link className="prev" to={previous.fields.slug}>
            ← Previous
          </Link>
        )}
        {next && (
